...
class AuthCubit extends Cubit<AuthStates> {
  ...
  static const String forgetPasswordApi =
      'https://innovahub-d3etetfzh6ada8aq.uaenorth-01.azurewebsites.net/api/Profile/generate-token'; // forget password api:
  static const String resetPasswordApi =
      '$baseUrl/resetPassword'; // reset password api:
  static const String rolesApi = '$baseUrl/roles'; // roles of users api:
  static const String googleLoginApi =
      '$baseUrl/googleLogin'; // googleLogin api:
  ...
  Future<void> register({
    ...
    try {
      final response = await http.post(
        // store the response that return from api:
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        emit(RegisterSuccessState(
            messagesuccess: 'Register Success')); // emit Success state:
      } else {
        var responseBody =
            jsonDecode(response.body); // convert json to map:(parsing):
        // emit failed:
        emit(RegisterErrorStata(
            message: responseBody['message'] ??
                'Failed to register')); // to access value of message:
      }
    } catch (e) {
